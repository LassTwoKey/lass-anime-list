      ...
      >
        ...
        <SwiperSlide key={item.id} style={{width: 210}}>
          <Link to={`anime/${item.id}`} className="flex flex-col gap-4">
            <img src={item.coverImage.large} className="max-h-72 rounded-lg overflow-hidden" alt="" />
            <h3 className="whitespace-nowrap overflow-hidden text-ellipsis text-green-500 text-xl hover:text-green-600 duration-150">{item.title.romaji}</h3>
          </Link>
        </SwiperSlide>
      ))}
      </Swiper>
